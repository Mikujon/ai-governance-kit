...
4. **Walk the correct checklist for the tier — don't shorten it.**
   ...
   - **Every tier:** also check `AI_PROJECT_GUIDELINES.md` Section 7 for tool-type and business-area follow-ups that apply — a tool can pick up a new one since it was first built (it added an LLM component, started touching payments, and so on), so re-check this on every audit, not just at intake.
