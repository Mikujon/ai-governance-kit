...
If any of these can't be satisfied, **stop and say so** — don't run a smaller, inferred, or "good enough" version of the audit instead. An audit that skipped its own rules to get an answer out isn't a lighter audit; it isn't an audit.
...
8. **Distribute the report** to whoever Section 5 names for this tier — never everyone, never just the owner regardless of tier. "Distribute" means tell the user who should see it; you're not assumed to have the means to email or message them yourself.
9. **Produce the remediation prompt** (Section 6) alongside the report whenever there's at least one Fail — the report says what's wrong; the remediation prompt is the separate, actionable file that gets it fixed.
