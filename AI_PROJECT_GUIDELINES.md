...
| **1** | Basic internal tool | A small script a team runs weekly, a personal report generator others start using, a Sheets/Apps Script automation that emails a scheduled report, a personal Zapier/Make flow | Light — business/technical contact named, Owner of record becomes the Governance Administrator (`GOVERNANCE.md` Section 2), basic security hygiene, no formal Hub record required but recommended |
...
| Owner of record = Governance Administrator (automatic, not chosen per project) | — (author) | ● | ● | ● |
| Business contact named | ○ | ● | ● | ● |
| Technical contact named | ○ | ● | ● | ● |
| Infrastructure contact named | — | — | ● | ● |
| RACI matrix | — | — | ○ | ● |
...
| Contact-handoff SLA on attrition (5 business days) | — | ○ | ● | ● |
...
- **[T3]** The business case is not filed-and-forgotten: it must be walked through out loud with the business contact's manager or the relevant governance seat (`GOVERNANCE.md`) before go-live, the same way the security sign-off is a real approval and not a checkbox.
...
### Ownership
- **[T0]** The author is the owner, full stop — nothing below this applies.
- **[T1+]** Owner of record is **not chosen per project** — it is the Chair / Governance Administrator seat, automatically, the moment a project crosses into T1 (`GOVERNANCE.md` Section 2). Do not put an individual employee's name in a Hub or Hub-equivalent "Owner" field for anything above T0.
- **[T1+]** Name a **business contact** (whoever requested it / whoever it serves) and a **technical contact** (whoever builds/maintains it) — even if they're the same person, write both down somewhere findable. These are working roles, not the accountable owner.
- **[T2+]** Also name an **infrastructure contact** — whoever owns the hosting/deploy environment the project runs on.
- **[T1+]** Contacts hand off within 5 business days of a role change or departure — never left pointing at someone who's gone. This is a lighter event than it sounds: the Owner of record doesn't change, only the working contact does, so there's no governance gap to backfill, just a handoff to log.
- **[T1+]** Create `OWNERSHIP_TRANSFER.md` from `OWNERSHIP_TRANSFER_TEMPLATE.md` and keep it current — access/credentials, current status, pending work, key contacts, and cost commitments a new contact would need on day one. People change roles and leave; a project's continuity can't depend on tribal knowledge held by one person who might not be reachable when the 5-day SLA clock starts. Review it at the same cadence as the project's own review reminder (below), not only when someone actually leaves.
- **[T1+]** The Governance Administrator's role here is connective, not custodial — the standing point of contact between the business contact, the technical contact, the infrastructure contact, and stakeholder functions (Security, Legal/Compliance). They don't do the work; they're who "who do I ask about this" always resolves to, regardless of who else has changed.
...
- **[T2+]** Register the tool in the AI Tools Governance Hub with Owner of record (Governance Administrator), business/technical/infrastructure contacts, scope, and data notes.
