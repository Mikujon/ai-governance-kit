...
- `PROCESS_FLOW.md` — l'intero kit ridisegnato come quattro diagrammi Mermaid: (1) intake → tier → starter, (2) gate di build per tier (T0–T3), (3) il ciclo dopo il go-live — revisione periodica e passaggio di ownership, disegnati come due loop separati che condividono lo stesso record, (4) il percorso di escalation di `GOVERNANCE.md` come flow invece che come tabella. Ogni riquadro rimanda al file che governa davvero quel passo. Linkato da `00_START_HERE.md` come punto di ingresso alternativo per chi preferisce vedere la forma intera prima di leggere file per file.
...
- `00_START_HERE.md`, `README.md` — documentata la nuova modalità "Option A, fully automatic" (skill Claude Code), aggiunte le righe per `OWNERSHIP_TRANSFER_TEMPLATE.md`, per lo skill e per `PROCESS_FLOW.md` nella tabella dei file; comandi di clone e URL raw aggiornati a `v1.4.0`.
