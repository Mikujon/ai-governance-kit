...
```bash
git clone --branch v1.13.0 https://github.com/wearefiber/ai-governance-kit.git
```

Or fetch a single file without cloning (useful inside a prompt):

```
https://raw.githubusercontent.com/wearefiber/ai-governance-kit/v1.13.0/AI_INTAKE_ASSESSMENT.md
```
...
| `AI_TECHNICAL_DEPTH_REVIEW.md` | Companion to the audit above, from Tier 1 up: a technical read — queries, caching, API maturity, scalability — that the governance audit deliberately doesn't do, scaled to the tier: a short spot-check at T1, full line-by-line depth at T3. |
